...
class GuildProtectionListener(
    ...
    override fun canPlaceBlock(player: Player, block: Block): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, block.chunk.toChunkCoord()))
    }
    ...
    override fun canOpenContainer(player: Player, container: Container): Boolean {
        if(Material.ENDER_CHEST == container.type) return true
            return (InteractionAllowResult.Deny
                    != guildService.canInteractInChunk(player.uniqueId,container.chunk.toChunkCoord()))
        }

    override fun canDamagePassiveEntity(player: Player, entity: LivingEntity): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, entity.chunk.toChunkCoord()))
    }
    ...
    override fun canFishEntity(player: Player, entity: Entity): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, entity.location.chunk.toChunkCoord()))
    }
    ...
    override fun canMountEntity(player: Player, mount: Entity): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, mount.location.chunk.toChunkCoord()))
    }

    override fun canEnterVehicle(player: Player, vehicle: Vehicle): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, vehicle.location.chunk.toChunkCoord()))
    }
    ...
    override fun canBreakHanging(player: Player, hanging: Hanging): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, hanging.location.chunk.toChunkCoord()))
    }

    override fun canEditItemFrame(player: Player, frame: ItemFrame, action: ItemFrameAction): Boolean {
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, frame.location.chunk.toChunkCoord()))
    }
    ...
    override fun canEditArmorStand(
        ...
        return (InteractionAllowResult.Deny
                != guildService.canInteractInChunk(player.uniqueId, stand.chunk.toChunkCoord()))
    }
